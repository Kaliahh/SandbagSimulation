...
public class Section
{
    ...
    public Vector3[] FindPlace(float viewDistance, Vector3 position, List<Vector3> constructionNodes)
    {
        ...
        List<Vector3> visited = new List<Vector3>();        // Brug til at afgøre om den indeholder flere næsten ens positioner
    ...
    {
        return position.x > constructionNodes[0].x && position.x < constructionNodes[constructionNodes.Count - 1].x ? true : false;
    }
}
